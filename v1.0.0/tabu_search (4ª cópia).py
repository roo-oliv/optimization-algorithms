				...
				if(hpw==2):
					...
					while(alloc2==0):
						...
						if(slots[period-1][b]==1 and waste==0):
							...
							if(control%2==1):
								b -= control
								if(b<0): control += 1; b += control
							else:
								b += control
								if(b>9): control += 1; b -= control
							if(waste==0 and not(0<=b<=9)):
								b = 5
								control = 0
								waste = 1
					...
					while(alloc3==0):
						if(c_mod==0):
							if(a%2==0 and a+1<=9): c = a+1
							elif(a-1>=0): c = a-1
							else: pass
						elif(c_mod==1):
							if(a%2==0 and b+1<=9): c = b+1
							elif(b-1>=0): c = b-1
							else:pass
						elif(c_mod==2):
							c = 0
						...
						if(slots[period-1][c]==1 and waste==0):
							...
							c += 1
						if(c==10 and (waste==0 and c_mod>2)):
							c = 0
							waste = 1
...
#-----------------------------------------------------------------------------------------#
	...
	for i in range(p):
		...
		if(book==1):
			for j in range(len(profBook[i])):
				for k in range(5,len(profBook[i][j])):
					prof_hpw += profBook[i][j][k]
		else: pass
		
		while(prof_hpw<8):
			attempt = 0
			sbj = profSubject[i][attempt]
			hpw = sbjHPW[sbj]
			period = sbjPeriod[sbj]
				...
				if(hpw==2):
					...
					try: b = pref_slots1[0]
						...
						for j in range(4,-1,-1):
							if(9>=a+j+mod>=0 and ((a+j+mod in pref_slots0) or (a+j+mod in pref_slots1))):
								b = a+j+mod
								break
						else:
							for j in range(4,-1,-1):
	...
	####	####	####	####	####	####	####	####	####	####
	a_test = 0								####
	for p_test in range(len(profBook)):					####
		for i_test in range(len(profBook[p_test])):			####
			for j_test in range(5,len(profBook[p_test][i_test])):	####
				a_test += 2					####
			if(j_test%2==1): a_test -= 1				####
	b_test = 0								####
	for i_test in range(len(profSche)):					####
		for j_test in range(len(profSche[i_test])):			####
			b_test += sum(profSche[i_test][j_test])			####
	if(b_test==a_test): pass						####
	else: print("Schedule Error!\na_test: "+str(a_test)+"\nb_test: "+str(b_test)); #y_fool = z_null				####
	####	####	####	####	####	####	####	####	####	####
	####	####	####	####	####	####	####	####	####	####
							...
							for m in range(len(sbjBook[sbj2])): # Pick one period and professor that will be offering this subject
								...
								for n in range(int(sbj2_info[2]/2)+sbj2_info[2]%2): # Iterate for all the slots that this subject occupies
									...
									for p_test in range(len(profBook)):					####
										for i_test in range(len(profBook[p_test])):			####
											for j_test in range(5,len(profBook[p_test][i_test])):	####
												a_test += profBook[p_test][i_test][j_test]	####
									b_test = 0								####
									for i_test in range(len(profSche)):					####
										for j_test in range(len(profSche[i_test])):			####
											b_test += sum(profSche[i_test][j_test])			####
				...
				for l in range(len(sbjBook[k])):
					...
					if(professor==professor2 or professor==-1 or professor2==-1): pass
						...
						if(sbj in profSubject[professor2] and sbj2 in profSubject[professor]):
							...
							for m in sbj2_slots:
								count += 1
								if(schedule[period2-1][m]==0 or (m in sbj_slots and period==period2) or (schedule[period2-1][m]==1 and count==len(info2)-5 and hpw2%2==1)):
									try: permute
									except: permute = 1
								else: permute = 0
							count = 0
							for n in sbj_slots:
								count += 1
								if(schedule2[period-1][n]==0 or (n in sbj2_slots and period==period2) or (schedule2[period-1][n]==1 and count==len(info)-5 and hpw%2==1)):
									try: permute
									except: permute = 1
								else: permute = 0
...
def tabuSearch(Itermax):
	...
	while(Iter-BestIter<Itermax):
		...
		if(prof1==-1): pass
			...
			try:                                                 #   ____________________
				for k in range(3):                           #  | Clean old schedule |
					pS1[operiod1][BestMove[0][5+k]] = 0  #  |  from professor 1  |
			except: pass                                         #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
		if(prof2==-1): pass
		else:
			try:                                                 #   ____________________
				for k in range(3):                           #  | Clean old schedule |
					pS2[operiod2][BestMove[1][5+k]] = 0  #  |  from professor 2  |
			except: pass                                         #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
		if(prof1==-1): pass
		else:
			try:                                                                 #   
				for k in range(3):                                           #  
					pS1[period1][BestMove[2][5+k]] = 2                   #   ____________________
				if(k==2): pS1[period1][BestMove[2][5+k]] = 1                 #  | Write new schedule |
				else: pass                                                   #  |  for professor 1   |
			except:                                                              #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
				if(BestMove[2][2]%2==1): pS1[period1][BestMove[2][4+k]] = 1  #
				else: pass                                                   #
		if(prof2==-1): pass
		else:
			try:                                                                 #   
				for k in range(3):                                           #  
					pS2[period2][BestMove[3][5+k]] = 2                   #   ____________________
				if(k==2): pS2[period2][BestMove[3][5+k]] = 1                 #  | Write new schedule |
				else: pass                                                   #  |  for professor 2   |
			except:                                                              #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
				if(BestMove[3][2]%2==1): pS2[period2][BestMove[3][4+k]] = 1  #
				else: pass                                                   #
		...
		if(osbj1==-1): pass
			...
			try:                                                 #   ____________________
				for k in range(3):                           #  | Clean old schedule |
					sS1[operiod1][BestMove[0][5+k]] = 0  #  |   from subject 1   |
			except: pass                                         #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
		if(osbj2==-1): pass
		else:
			try:                                                 #   ____________________
				for k in range(3):                           #  | Clean old schedule |
					sS2[operiod2][BestMove[1][5+k]] = 0  #  |   from subject 2   |
			except: pass                                         #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
		if(sbj1==-1): pass
		else:
			try:                                                                 #   
				for k in range(3):                                           #  
					sS1[period1][BestMove[2][5+k]] = 2                   #   ____________________
				if(k==2): sS1[period1][BestMove[2][5+k]] = 1                 #  | Write new schedule |
				else: pass                                                   #  |   for subject 1    |
			except:                                                              #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
				if(BestMove[2][2]%2==1): sS1[period1][BestMove[2][4+k]] = 1  #
				else: pass                                                   #
		if(sbj2==-1): pass
		else:	
			try:                                                                 #   
				for k in range(3):                                           #  
					sS2[period2][BestMove[3][5+k]] = 2                   #   ____________________
				if(k==2): sS2[period2][BestMove[3][5+k]] = 1                 #  | Write new schedule |
				else: pass                                                   #  |   for subject 2    |
			except:                                                              #   ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
				if(BestMove[3][2]%2==1): sS2[period2][BestMove[3][4+k]] = 1  #
				else: pass                                                   #
