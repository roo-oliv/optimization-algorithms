...
def TabuSearch(Itermax, sbjHPW, sbjGroup, sbjBook, profBook, profSche, profSubject, profPeriod):
	...
	print("   |\tTry over cost increase\t|\tGlobal best cost\t|\tCurrent solution's cost\t     |")
	while(Iter-BestIter<Itermax):
		Iter += 1
		print("\r   |\t\t",Iter-BestIter,"\t\t|\t\t",global_best,"\t\t|\t\t",liquid_cost,"\t\t     |", end='')
		global_best_before = global_best
		BestMove, BestIter, global_best, liquid_cost = get.Best(BestIter, [], T, Ti, sbjHPW, sbjGroup, sbjBook, profBook, profSche, profSubject, profPeriod, global_best, liquid_cost)
		...
		# Update best solution if current solution is the actual global best
		if(global_best != global_best_before):
			profSche_best = copy.deepcopy(profSche)
			profBook_best = copy.deepcopy(profBook)
			sbjBook_best = copy.deepcopy(sbjBook)

	return sbjBook, profBook, profSche, sbjBook_best, profBook_best, profSche_best, BestMove_log
