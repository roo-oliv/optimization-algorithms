...
# Assign professors for alternate schedules in subject groups ( Classes B )
	...
	for i in range(len(sbjGroups)): # iterate for all subject groups
		for j in range(5): # each group is composed by 5 subjects
			...
			for period in periods:
				for A_info in sbjBook[sbj]:
					if(A_info[1]==period):
						info = copy.deepcopy(A_info)
						for k in range(len(A_info[5:])):
							info[5+k] = A_info[5+k]-2*(A_info[5+k]%2)+1
						break
					else: pass
				else: raise Exception("Subject not found in sbjBook.")
				profPos = 0
				force = 0
				next_candidate = 1
				while(next_candidate==1):
					...
					for k in range(len(info[5:])):
						slot = info[5+k]
						if(hpw%2 == 1 and k+6 == len(info)): requirement = 1
						else: requirement = 2
						if(profSche[professor][period][slot]+requirement <= 2):
							ok += 1
							break
						else: print(professor,profSche[professor],period,slot,requirement)
					
					if(ok==len(info[5:])): next_candidate = 0
					else: profPos += 1
				...
				try: print('Professor %i was assigned to teach subject %i on' % (professor,sbj),message[0],message[1],message[2],'\b.')
				except: print(message,info,A_info); raise
		...
		for A_info in sbjBook[-1]:
			if(A_info[4]==i):
				info = copy.deepcopy(A_info)
				for j in range(len(A_info[5:])):
					info[5+j] = A_info[5+j]-2*(A_info[5+j]%2)+1
					
				sbjBook[-1].append(info)
				profBook[-1].append(info)
...
# Assign professors to subjects not constrained to subject groups
	...
	for professor in range(p):
		...
		while(prof_hpw < 8):
			...
			allocated_slots = []
			info = [sbj,period,hpw,professor,None]
			...
			for k in (6,7):
				...
				try:
					...
					try:
						...
						info[k+1]
						message[k-6] = message[k-6]+", on"
					except: print("except int"); message[k-6] = message[k-6]+" and on"
				except:
					print("except ext")
					message.append('\b')
			print(message)
